...
static const int TAMANIO_MAXIMO_busqueda = 100;
...
{
    return (a->cantidad_elementos == TAMANIO_MAXIMO_busqueda);
};
...
int obtenerAltura(NodoArbol nodo)
{
    ...
    int altura_izquierda = obtenerAltura(n_hijoizquierdo(nodo));
    int altura_derecha = obtenerAltura(n_hijoderecho(nodo));
    return (altura_izquierda > altura_derecha) ? altura_izquierda + 1 : altura_derecha + 1;
}

bool abb_EstaEquilibrado(ArbolBinarioBusqueda A)
{
    if (A == NULL || abb_es_vacio(A))
    {
        return true; // Un árbol vacío es considerado equilibrado
    }

    int altura_izquierda = obtenerAltura(n_hijoizquierdo(A->raiz));
    int altura_derecha = obtenerAltura(n_hijoderecho(A->raiz));
    int diferencia_alturas = abs(altura_izquierda - altura_derecha);

    return diferencia_alturas <= 1;
}